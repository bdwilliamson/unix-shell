...
\begin{flushleft}
\textit{GUIs are picture books --- they let you move things to other things, or right click on things and select options.}
\vspace{0.1cm}

\textit{But you still type a post when you want to ask a question.}
\vspace{0.1cm}

\textit{Command lines are much the same thing... if what you're trying to do is simple, then pictures and words are about the same. If what you're trying to do is complex, then words allow you to explain better.}
\end{flushleft}
...
Permutation tests construct a \textcolor{blue}{sampling distribution for a test statistic}, like the bootstrap.
...
Some code to set this up: \tiny{(from \href{http://faculty.washington.edu/kenrice/sisg/indexSEA09.shtml}{Ken's} SISG course)}
\begin{verbatim}
